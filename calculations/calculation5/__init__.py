...
@exp5_bp.route('/')
def index():
    return render_template('exp5/mixture_conductometry.html')
...
def calculate():
    ...
    if len(volumes) < 5 or len(conductance) < 5:
        return render_template('exp5/mixture_conductometry.html', error="Please enter at least 5 readings")
    ...
    return render_template('exp5/mixture_conductometry_intermediate.html',
                         v1=round(v1, 2),
                         v2=round(v2, 2),
                         v3=round(v2 - v1, 2),
                         graph_data=json.dumps(graph_data))

@exp5_bp.route('/final_result', methods=['POST'])
def final_result():
    ...
    return render_template('exp5/mixture_conductometry_result.html',
                         volumes=volumes,
                         conductance=conductance,
                         v1=round(v1, 2),
                         v2=round(v2, 2),
