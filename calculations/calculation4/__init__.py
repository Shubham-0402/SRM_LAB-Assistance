...
@exp4_bp.route('/')
def index():
    return render_template('exp4/conductometry.html')

@exp4_bp.route('/calculate', methods=['POST'])
def calculate():
    ...
    if len(volumes) < 3 or len(conductance) < 3:
        return render_template('exp4/conductometry.html', error="Please enter at least 3 readings")
    ...
    return render_template('exp4/conductometry_result.html',
                         volumes=volumes,
                         conductance=conductance,
                         ve=round(ve, 2),
                         norm_naoh=norm_naoh,
